...
pdur = 0.05  # pip duration
pstart = [0.01]  # pip start times
rt = 2.5e-3 # rise/fall time in seconds
stimdb = 65 # stimulus intensity in dB SPL

CFL = np.logspace(np.log10(200), np.log10(6000), 5);  # list of CFs
F0L = CFL;  # test frequency matches CFs (same list)
SPL = 60;  # from 0 to 90 dbSPL in 10 dB steps
nrep = 2;
...
t = np.arange(len(vihc)) * (1./Fs) # 1e-5
p2.plot(t, vihc)

p3 = win.addPlot(col=1, row=0, title='ANF PSTH')
p3.setXLink(p2)
ds = 100
size = psth.size // ds
rpsth = psth[:size*ds].reshape(size, ds).sum(axis=1)
t = np.arange(len(rpsth)) * 1./Fs * ds # 1e-5 * ds
p3.plot(t, rpsth[:-1], stepMode=True, fillLevel=0, fillBrush='w')

p4 = win.addPlot(col=1, row=1, title='ANF Sync')
p3.setXLink(p3)
