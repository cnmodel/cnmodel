	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 k_conc /Users/pbmanis/Desktop/Python/nrnlibrary/nrnlibrary/i386/K_conc.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dki = - ( ik ) / ( 2.0 * FARADAY * d ) * ( 1e4 ) - beta * ( ki - ki0 ) ;
   Dko = ( ik ) / ( 2.0 * FARADAY * d ) * ( 1e4 ) - beta * ( ko - ko0 ) ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dki = Dki  / (1. - dt*( ( - (beta)*(( 1.0 )) ) )) ;
 Dko = Dko  / (1. - dt*( ( - (beta)*(( 1.0 )) ) )) ;
}
 /*END CVODE*/
